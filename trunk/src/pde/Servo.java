...
public class Servo extends Signal {
  ...
  public Servo(PApplet applet, String label, int left, int top, int width, int height) {
    ...
    panel.setLabelBottomHeight((2 * Fonts.kLineHeight) + Fonts.kLineSpacing);
  }
  ...
  public void draw() {
    ...
    // servo case
    applet.fill(Colors.kColorComponent);
    ...
    applet.text(label, panel.labelBottom.getWidth() - Fonts.kLineSpacing, Fonts.kLineHeight);
    // draw the labesl's value
    applet.textFont(Fonts.getFontPlain(applet), Fonts.kFontSize);
    applet.textAlign(PApplet.RIGHT);
    applet.text(String.format("%1.3f", getValue()), panel.labelBottom.getWidth() - Fonts.kLineSpacing, 2 * Fonts.kLineHeight);
    applet.popMatrix();
